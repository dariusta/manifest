...
import { CLAUDE_CODE_PROVIDER_OWNED_HEADERS } from '../../common/constants/subscription-clients';
    ...
    if (endpointKey === 'anthropic' && authType === 'subscription') {
      for (const protectedName of CLAUDE_CODE_PROVIDER_OWNED_HEADERS) {
        const providerOwnedValue = Object.entries(headers).find(
          ([key]) => key.toLowerCase() === protectedName,
        )?.[1];
        for (const key of Object.keys(finalHeaders)) {
          if (key.toLowerCase() === protectedName) delete finalHeaders[key];
        }
