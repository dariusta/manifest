...
export class ProviderClient {
  ...
  async forward(opts: ForwardOptions): Promise<ForwardResult> {
    ...
    // This identifies the forwarding Manifest server to Anthropic subscription
    // backends. It is generated by Manifest and must never be caller-controlled.
    if (headers['x-forwarded-server']) {
      finalHeaders['x-forwarded-server'] = headers['x-forwarded-server'];
    }
