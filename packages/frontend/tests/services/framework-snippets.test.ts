...
describe('TOOLKIT_TABS', () => {
  ...
  it('contains openai-sdk, anthropic-sdk, google-genai-sdk, vercel-ai-sdk, langchain, curl', () => {
  ...
  it('has display labels', () => {
    expect(TOOLKIT_TABS.map((t) => t.label)).toEqual([
      'OpenAI SDK',
      'Anthropic SDK',
      'Google Gen AI SDK',
      'Vercel AI SDK',
  ...
  it('has icons for every tab except cURL', () => {
    expect(TOOLKIT_TABS.map((t) => t.icon)).toEqual([
...
describe('getClaudeCodeSettingsSnippet', () => {
  it('emits a paste-ready JSON block for ~/.claude/settings.json', async () => {
    const { getClaudeCodeSettingsSnippet } = await import('../../src/services/framework-snippets');
...
describe('getNanobotConfigSnippet', () => {
  it('emits a paste-ready JSON block for ~/.nanobot/config.json', async () => {
    const { getNanobotConfigSnippet } = await import('../../src/services/framework-snippets');
...
describe('getSnippetForToolkit', () => {
  ...
  it('strips trailing /v1 from base_url for the Google Gen AI Python SDK', () => {
    // The SDK composes `{base_url}/{api_version}/models/...` itself, so a
    // trailing /v1 would yield /v1/v1beta/models and a 404.
    const result = getSnippetForToolkit('google-genai-sdk', 'http://x/v1', 'key', 'python');
    expect(result.title).toBe('Google Gen AI Python SDK');
    expect(result.code).toContain('from google import genai');
    expect(result.code).toContain('http_options={"base_url": "http://x"}');
    expect(result.code).not.toContain('http://x/v1');
    expect(result.code).toContain('client.models.generate_content');
  });

  it('strips trailing /v1 from baseUrl for the Google Gen AI TypeScript SDK', () => {
    const result = getSnippetForToolkit('google-genai-sdk', 'http://x/v1', 'key', 'typescript');
    expect(result.title).toBe('Google Gen AI TypeScript SDK');
    expect(result.code).toContain('import { GoogleGenAI } from "@google/genai"');
    expect(result.code).toContain('httpOptions: { baseUrl: "http://x" }');
    expect(result.code).not.toContain('http://x/v1');
    expect(result.code).toContain('ai.models.generateContent');
  });

  it('defaults the Google Gen AI SDK to Python', () => {
    const result = getSnippetForToolkit('google-genai-sdk', 'https://gw.example.com', 'key');
    expect(result.title).toBe('Google Gen AI Python SDK');
    expect(result.code).toContain('"base_url": "https://gw.example.com"');
  });
...
describe('getLangForToolkit', () => {
  ...
  it('follows the shared language toggle for anthropic-sdk', () => {
    expect(getLangForToolkit('anthropic-sdk')).toBe('python');
    expect(getLangForToolkit('anthropic-sdk', 'typescript')).toBe('typescript');
  });

  it('follows the shared language toggle for google-genai-sdk', () => {
    expect(getLangForToolkit('google-genai-sdk')).toBe('python');
    expect(getLangForToolkit('google-genai-sdk', 'python')).toBe('python');
    expect(getLangForToolkit('google-genai-sdk', 'typescript')).toBe('typescript');
  });
...
describe('customHeaders weaving', () => {
  ...
  it('google genai: nests custom headers inside http_options / httpOptions', () => {
    const py = getSnippetForToolkit(
      'google-genai-sdk',
      'https://api.local/v1',
      'k',
      'python',
    ...
    expect(py.code).toContain('"headers": {"x-manifest-tier": "premium"}');
    const ts = getSnippetForToolkit(
      'google-genai-sdk',
      'https://api.local/v1',
      'k',
      'typescript',
    ...
    expect(ts.code).toContain('headers: { "x-manifest-tier": "premium" }');
  });

  it('google genai: omitting customHeaders leaves the snippet unchanged', () => {
    const py = getSnippetForToolkit('google-genai-sdk', 'https://api.local/v1', 'k', 'python');
    expect(py.code).not.toContain('headers');
    const ts = getSnippetForToolkit('google-genai-sdk', 'https://api.local/v1', 'k', 'typescript');
    expect(ts.code).not.toContain('headers:');
  });
