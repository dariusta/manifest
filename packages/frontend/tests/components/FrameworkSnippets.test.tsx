...
/**
 * Click a toolkit tab by its visible label. Positional indices silently
 * repoint at a neighbouring tab whenever TOOLKIT_TABS grows.
 */
function clickTab(container: HTMLElement, label: string): void {
  const tab = [...container.querySelectorAll('.panel__tab')].find((t) =>
    t.textContent?.includes(label),
  );
  if (!tab) throw new Error(`no toolkit tab labelled "${label}"`);
  fireEvent.click(tab);
}
...
describe('FrameworkSnippets', () => {
  ...
  it('renders one tab per toolkit', () => {
    const { container } = render(() => <FrameworkSnippets {...defaultProps} />);
  ...
  it('switches to Anthropic SDK tab on click', () => {
    const { container } = render(() => <FrameworkSnippets {...defaultProps} />);
    clickTab(container, 'Anthropic SDK');
  ...
  it('switches to Vercel AI SDK tab on click', () => {
    const { container } = render(() => <FrameworkSnippets {...defaultProps} />);
    clickTab(container, 'Vercel AI SDK');
  ...
  it('renders tab icons for tabs that have them', () => {
    ...
    // Every toolkit but cURL ships an icon.
