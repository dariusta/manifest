...
async function loadWorkspaces() {
  try {
    ...
    // The header is reused in tests and during rolling deploys where an older
    // backend can return a non-workspace payload. Treat that as unavailable
    // instead of allowing the global header to crash.
    return Array.isArray(response?.workspaces) ? response : null;
  } catch {
    return null;
  }
}
...
const WorkspaceSwitcher: Component = () => {
  const [data, { refetch }] = createResource(loadWorkspaces);
